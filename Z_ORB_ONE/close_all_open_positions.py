...
def sum_inventory_shares(inventory: dict[str, Any]) -> int:
    stock_details = inventory.get("stk_dats") or []
    if isinstance(stock_details, list) and stock_details:
        return sum(parse_int(item.get("qty")) for item in stock_details if isinstance(item, dict))

    return parse_int(inventory.get("qty"))
...
def load_inventory_positions(sdk) -> Optional[list[InventoryPosition]]:
    ...
    for inventory in inventories:
        ...
        shares = sum_inventory_shares(inventory)

        if not stock_no or shares <= 0:
            continue

        if shares % SHARES_PER_LOT != 0:
            print(f"[WARN] {stock_name or stock_no} 庫存股數 {shares} 不是 1000 的整數倍，略過避免下錯張數")
            continue
...
def ask_trade_side() -> str:
    while True:
        answer = input("請輸入目前全部部位方向 SHORT 或 LONG: ").strip().upper()
        if answer in (TRADE_SIDE_SHORT, TRADE_SIDE_LONG):
            return answer
        print("請輸入 SHORT 或 LONG。")


def close_position(position: InventoryPosition, sdk, side: str) -> bool:
...
def main() -> int:
    ...
    side = ask_trade_side()
    print(f"開始以 {side} 方向平倉，共 {len(positions)} 檔")

    success_count = 0
    failed_positions: list[InventoryPosition] = []
    for position in positions:
        print(f"[{position.stock_name or position.stock_no}] 平倉 {position.lots} 張")
        if close_position(position, sdk, side):
            success_count += 1
    ...
    print(f"平倉委託完成: 成功 {success_count} 檔，失敗 {len(failed_positions)} 檔")
    if failed_positions:
