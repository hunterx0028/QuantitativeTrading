...
ENTRY_CHECK_END_TIME = (10, 1)  # 進場檢核截止時間（含）
...
def build_initial_state(
    ...
    return {
        ...
        "trigger_low_price": v3,  # 觸發進場價，可能比昨收還低
...
def are_recent_lows_above_or_equal_yesterday_low(
    ...
    trigger_low_price = state.get("trigger_low_price")
    ...
    try:
        trigger_low = float(trigger_low_price)
    except (TypeError, ValueError):
        return False
    ...
    for candle in recent_candles:
        ...
        try:
            if float(low_value) < trigger_low:
                return False
        except (TypeError, ValueError):
            return False
...
def entry_price_check(state: Dict[str, Any], realtime_sdk: EsunMarketdata) -> bool | str:
    ...
    if low_price < trigger_low_price:  # 跌破昨低即進場
        ...
        if not is_intraday_range_within_threshold_before_trigger(completed_candles, float(yesterday_low_price)):
            print(f"[{state['symbol_name']}] {now_local.strftime('%H:%M:%S')} 已跌破入場價，但觸發前高低價差過大，不追蹤")
            return 'BLOCKED'

        if not are_recent_lows_above_or_equal_yesterday_low(completed_candles, state):
            print(f"[{state['symbol_name']}] {now_local.strftime('%H:%M:%S')} 之前K棒未能保持在入場價之上，突破失敗，不追蹤")
            return 'BLOCKED'

        latest_average, _latest_volume = get_latest_complete_candle_average_and_volume(completed_candles)
        if (latest_average is None) or (latest_average <= float(trigger_low_price)):
            print(f"[{state['symbol_name']}] {now_local.strftime('%H:%M:%S')} 均價小於入場價，突破失敗，不追蹤")
            return 'BLOCKED'
        ...
        for st in states.values():
            ...
            try:
                ...
                if round_should_update_realtime:
                    ...
                    trigger_low_price = st.get("trigger_low_price")
                    if (
                    ...
                    if (
                        ((now_local.hour, now_local.minute) < ENTRY_CHECK_START_TIME)
                        and (trigger_low_price is not None)
                        and (low_price is not None)
                        and (low_price < trigger_low_price)
                        ):
                        ...
                        st["trigger_low_price"] = low_price
                        print(f"[{st['symbol_name']}] {now_tpe().strftime('%H:%M:%S')} 檢核時間前已先突破，更改入場價為 {low_price}")
                    ...
                    if (st.get("in_position")) and (not st.get("traded")):  # 已持倉嘗試平倉
                        ...
                        if entry_result is True:
                            ylow = float(st.get("trigger_low_price", 0))
                            st["side"] = "SHORT"
