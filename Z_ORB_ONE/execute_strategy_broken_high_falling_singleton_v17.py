...
class TeeStream:
    ...
    def type_place_order(mysdk, symbol_code_with_suf, action_type, trade_type, quantity=1, price_flag=PriceFlag.Market,
                         price=0.0) -> Optional[bool]:
        priceInfo = price
    ...
    def calculate_range_fraction_prices(entry_price: float, pre_close_price: float, trade_strategy: str) -> Tuple[
        float, float]:
        """
    ...
    def atomic_write_json(path: str, data: Dict[str, Any]):
        ...
        with NamedTemporaryFile("w", delete=False, dir=os.path.dirname(abs_path), suffix=".tmp",
    ...
    def entry_price_check(state: Dict[str, Any], realtime_sdk: EsunMarketdata) -> bool | str:
        ...
        if low_price < yesterday_low_price:  # 跌破昨低即進場
            ...
            if not is_intraday_range_within_threshold_before_trigger(completed_candles, float(yesterday_low_price)):
                print(
                    f"[{state['symbol_name']}] {now_local.strftime('%H:%M:%S')} 已跌破昨低，但觸發前高低價差過大，不追蹤")
                return 'BLOCKED'

            if not are_recent_lows_above_or_equal_yesterday_low(completed_candles, state):
                print(
                    f"[{state['symbol_name']}] {now_local.strftime('%H:%M:%S')} 之前K棒未能保持在昨低之上，突破失敗，不追蹤")
                return 'BLOCKED'
    ...
    def try_open_position(state: Dict[str, Any], mysdk):
        ...
        if check_open_status(state):
            if state.get("side") == "SHORT":  # SHORT 作空
                ...
                place_order_result = type_place_order(mysdk, state["symbol_code_with_suf"], Action.Sell,
                                                      Trade.DayTradingSell, quantity=qty, price_flag=PriceFlag.Market,
                                                      price=entry_ref_px)
                if place_order_result:  # 下單成功
                    ...
                    candidate_profit = max(state.get("entry_price", 0) - open_profit_target,
    ...
    def try_close_position(state: Dict[str, Any], mysdk):
        ...
        if sl:
            ...
            print(
                f"[{state['symbol_name']}] ✅ 動態調整停利價格 停利：{state['stop_profit_price']} 下個目標：{state['profit_price']}")
        elif sp:
        ...
        if not exit_place_result:
            if side == "SHORT":
                limit_order_result = type_place_order(mysdk, state["symbol_code_with_suf"], Action.Buy, Trade.Cash,
                                                      quantity=state.get("qty", 0), price_flag=PriceFlag.LimitUp,
                                                      price=0)
    ...
    def initialize_states(
        ...
        for symbolStr, qty, v1, v2, v3, v4, industry_code, volatility_value, streak_tuple in stocks:
            ...
            try:
                limit_up_price, limit_down_price, symbol_can_buy_day_trade = get_up_down_price(code_with_suf,
                                                                                               realtime_sdk)
            except Exception as e:
    ...
    def monitor(states: Dict[str, Dict[str, Any]], mysdk: SDK, realtime_sdk: EsunMarketdata):
        ...
        while True:
            ...
            if (not entry_check_end_announced) and (
                    (now_local.hour, now_local.minute, now_local.second) >= (ENTRY_CHECK_END_TIME[0],
                                                                             ENTRY_CHECK_END_TIME[1], 0)):
            ...
            for st in states.values():
                ...
                try:
                    ...
                    if round_should_update_realtime:
                        try:
                            # print("更新股價")
                            px, open_px, high_price, low_price, close_price, avg_price = get_realtime_price(
                                st.get("symbol_code_with_suf", ""), realtime_sdk)
                        except Exception as e:
                        ...
                        if (
                                ...
                                and (
                        not is_intraday_range_within_threshold_by_realtime_prices(float(yesterday_low_price),
                                                                                  float(high_price), float(low_price)))
                        ):
                            st["traded"] = True
                            st["entry_time"] = now_tpe().isoformat()
                            atomic_write_json(state_path(st.get("symbol_code_with_suf", "")), st)
                        ...
                        if (
                            ...
                            atomic_write_json(state_path(st.get("symbol_code_with_suf", "")), st)
                            print(
                                f"[{st['symbol_name']}] {now_tpe().strftime('%H:%M:%S')} 檢核時間前已先突破昨低，不追蹤")
                            continue
            ...
            if (now_local.hour, now_local.minute) >= FORCE_EXIT_TIME:
                print(
                    f"當前時間 {now_local.strftime('%Y-%m-%d %H:%M:%S')} >= {FORCE_EXIT_TIME[0]:02d}:{FORCE_EXIT_TIME[1]:02d}，程式結束。")
                sys.exit(0)
