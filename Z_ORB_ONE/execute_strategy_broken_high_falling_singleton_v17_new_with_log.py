...
def log_entry_observation_if_triggered(state: Dict[str, Any], now_local: datetime, context: str) -> bool:
    """觀測原買一與盤勢濾網條件，不影響實際進場判斷。"""
    yesterday_low_price = state.get("yesterday_low_price")
    last_price = state.get("last_price")
    if yesterday_low_price is None or last_price is None:
        return False
    ...
    if last_px > trigger_price:
        return False

    best_bid_price = state.get("best_bid_price")
    try:
        best_bid = float(best_bid_price)
        best_bid_pass = best_bid <= trigger_price
        best_bid_text = str(best_bid)
    except (TypeError, ValueError):
        best_bid = None
        best_bid_pass = False
        best_bid_text = "None"

    market_pass = market_trend_filter_pass(state)
    print(
        f"[{state['symbol_name']}] {now_local.strftime('%H:%M:%S')} "
        f"{context} last_price已跌破進場觸發價，觀測條件："
        f"best_bid_pass={best_bid_pass} market_trend_filter_pass={market_pass} "
        f"last_price={last_px} best_bid={best_bid_text} "
        f"trigger_price={trigger_price} max_slippage_price={max_slippage_price}"
    )
    return True
...
def entry_price_check(state: Dict[str, Any], realtime_sdk: EsunMarketdata) -> bool | str:
    ...
    if last_px <= trigger_price:  # 成交價跌破昨低下一檔才進場，買一與盤勢濾網僅觀測
        log_entry_observation_if_triggered(state, now_local, "入場候選")
            ...
            try:
                ...
                if round_should_update_realtime:
                    ...
                    if (st.get("in_position")) and (not st.get("traded")):  # 已持倉嘗試平倉
                        if (ENTRY_CHECK_START_TIME <= (now_local.hour, now_local.minute) <= ENTRY_CHECK_END_TIME):
                            log_entry_observation_if_triggered(st, now_local, "持倉中再次觸發")
